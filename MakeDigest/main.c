...
char allchars[70]="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890";

int printHelp() {
    
    printf("Usage: makedigest.exe -w word\n");
...
{
  int one,two,three,four,five,maxValue,lowest_distance;
  FILE *fp;
  char extra[6];
  char baseMessage[200];
  char message[206];
  char arg[21];
  char target[21];
  char outputFile[21];  
  ...
  if (argc > 1) {
		...
		while (i < argc) {
			...
			if (strcmp(arg, "-w") == 0) {
                i++;
				if (strlen(argv[i]) == 0) {
                  printf("ERROR: You need to enter a word to generate digests for!\n");
                  return 0;
                }
                else if (strlen(argv[i]) > 20) {
                  printf("ERROR: Your word is too long (max length 20).\n");
                  return 0;
                }
                else {
                ...
                if (strlen(argv[i]) == 0) {
                  ...
                  printf("ERROR: The passed digest must be 40 characters.\n");
                  return 0;
                }
                else {
                  strcpy(targetDigest,argv[i]);
                  i++;
                  continue;
                }     
            }
  ...
  i = hex2bin(targetDigest, b);
  
  sprintf(outputFile,"%s.txt",target);
  ...
  strcpy(baseMessage,target);
  for (i = 0; i < 11; i++) {
      strcat(baseMessage," ");
      strcat(baseMessage,target);
  }
      
  maxValue = strlen(allchars);

  for(one = 0;one < maxValue; one++) {
     for(two = 0;two < maxValue; two++) {  
        for(three = 0; three < maxValue; three++) {
           for(four = 0; four < maxValue; four++) {       
              for(five = 0; five < maxValue; five++) {   
                strcpy(message,baseMessage);
                sprintf(extra, " %c%c%c%c%c",allchars[one],allchars[two],allchars[three],allchars[four],allchars[five]);
                strcat(message,extra);
                
                SHAInit(&sha);
	            SHAUpdate(&sha, message, strlen(message));
	            SHAFinal(digest, &sha);

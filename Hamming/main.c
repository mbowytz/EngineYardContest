...
char * randomString(long salt, char* mystring) {
     char my_letters[110] = "aAbBcDdEefgFhGjkHlIrJstuvwxyz12a345E6789a0ABCEDEFaGHeIJKLMnNOPrQRSTUVWXYZ";
     char randChar[2]="";
     int counter = 0;
     srand((unsigned)time(NULL)+salt+salt);
      ...
      {          
          strncpy(randChar,my_letters+rand()%strlen(my_letters),1);
 ...
 j = 0; 
  ...
  while(lowest_distance > 0) {   
    ...
    for(k = 0; k < 10; k++) {
      ...
      strcpy(message,randomWords(wordcount,j+k,randomWordString));
